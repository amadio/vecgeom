...
/*
VECGEOM_CUDA_HEADER_BOTH
bool UnplacedHype::Normal(Vector3D<Precision> p,Vector3D<Precision> &normal){
...
void UnplacedHype::GetParametersList(int, double* aArray)const
  ...
  cosphi = std::cos(phi);
  sinphi = std::sin(phi);
  sinhu = RNG::Instance().uniform(-1.*fDz*fTOut/fRmax,
                          fDz*fTOut/fRmax);
  ...
  if(chose>=0. && chose < aOne)
  {
    ...
    {
      return Vector3D<Precision>(fRmax*cosphi,fRmax*sinphi,
                           RNG::Instance().uniform(-fDz,fDz)); //RandFlat::shoot
    }
  }
  else if(chose>=aOne && chose<aOne+aTwo)
  {
    if(fStIn != 0.)
    {
      sinhu = RNG::Instance().uniform(-1.*fDz*fTIn/fRmin,
                              fDz*fTIn/fRmin);
