...
void ScanGeometry( VPlacedVolume const *const volume,
    ...
    if( std::find( lvlist.cbegin(), lvlist.cend(), volume->GetLogicalVolume() ) == lvlist.cend()
	 ...
	 cout << "Logical Volume " << volume->GetLogicalVolume()->GetName() << " has no tracking medium" << endl;
      }
  ...
  if ( std::find( tlist.cbegin(),tlist.cend(), volume->GetTransformation()) == tlist.cend() ){
    ...
    void GeomCppExporter::DumpMaterials( std::vector<std::stringstream *> & materials,
        ...
        unsigned int group = 0;
        std::stringstream * newstream = new std::stringstream;
        materials.push_back( newstream );
        std::stringstream * materialconstr = materials.back();
        materialdecl.push_back( new std::stringstream );
        ...
        *materialconstr << "void GenerateMaterials_part" << group << "(){\n";
        for ( auto ma : fMaterialToStringMap ){
            ...
            if(++groupcounter > 5000){
                ...
                *materialconstr << "}\n";
                ...
                // create a new stream
                materials.push_back( new std::stringstream );
	    ...
	    if(nel == 1 ) {
	       ...
	       line << "\"" << ma.first->GetName() << "\","
		    << ma.first->GetA() << "," << ma.first->GetZ() << "," 
		    << ma.first->GetDensity() << ");\n";
	    } else {
	       if(nel > 100) {
		  cout << "GeomCppExporter::" << __func__ 
		       << ": Cannot handle more than 100 elements in a material" << endl;
		  exit(1);
	       }
	       double mata[100];
	       double matz[100];
    ...
    void GeomCppExporter::DumpMedia( std::vector<std::stringstream *> & media,
        ...
        unsigned int group = 0;
        std::stringstream * newstream = new std::stringstream;
        media.push_back( newstream );
        std::stringstream * mediumconstr = media.back();
        mediumdecl.push_back( new std::stringstream );
        ...
        *mediumconstr << "void GenerateMedia_part" << group << "(){\n";
        for ( auto ma : fMediumToStringMap ){
            ...
            if(++groupcounter > 5000){
                ...
                *mediumconstr << "}\n";
                ...
                // create a new stream
                media.push_back( new std::stringstream );
	    ...
	    if(fMaterialToStringMap.find(mm) == fMaterialToStringMap.end()) {
	       cout << "Could not find material " << mm->GetName() << endl;
	       exit(1);
	    }
	    line << ma.second << " = new Medium(";
	    line << "\"" << ma.first->GetName() << "\","
		 << fMaterialToStringMap[mm] << ", nullptr);\n";
...
//                }
	...
	if(m) {
	   if(fMediumToStringMap.find(m) == fMediumToStringMap.cend()) {
	      cout << "Could not find medium " << m->GetName() << endl;
	      exit(1);
	   }
	   line << fLVolumeToStringMap[l] << "->SetTrackingMediumPtr(static_cast<void*>("
		<< fMediumToStringMap[m] << "));\n";
	}
...
void GeomCppExporter::DumpGeometry( std::ostream & s ) {
  // stringstreams to assemble code in parts
    ...
    for( unsigned int i = 0; i< materialdecl.size(); ++i) {
        std::ofstream outfile;
        std::stringstream name; name << "geomconstr_materials_part" << i << ".cpp";
        outfile.open(name.str());
    ...
    for( unsigned int i = 0; i< mediumdecl.size(); ++i) {
        std::ofstream outfile;
        std::stringstream name; name << "geomconstr_media_part" << i << ".cpp";
        outfile.open(name.str());
        outfile << "#include \"materials/Material.h\"\n";
