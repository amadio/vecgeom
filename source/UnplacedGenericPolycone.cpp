...
{
  if (placement) {
    new (placement) SpecializedGenericPolycone<trans_code, rot_code>(logical_volume, transformation, id);
    return placement;
  }
  return new SpecializedGenericPolycone<trans_code, rot_code>(logical_volume, transformation, id);
}
