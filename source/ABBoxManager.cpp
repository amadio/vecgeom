...
// Splitted Aligned bounding boxes
	void ABBoxManager::ComputeSplittedABBox(VPlacedVolume const *pvol,
			std::vector<ABBox_t> &lowerc,
		...
		for (int i = 0; i < numOfSlices; i++) {
			...
			if (dim == 0) {
				transVec =
				transf->InverseTransform(Vector3D<Precision>(val, 0., 0.));
				val += splitDx;
			}
			if (dim == 1) {
				transVec =
				transf->InverseTransform(Vector3D<Precision>(0., val, 0.));
				val += splitDy;

			}
			if (dim == 2) {
				transVec =
				transf->InverseTransform(Vector3D<Precision>(0., 0., val));
				val += splitDz;
			...
			transf2.SetRotation(
					transf->Rotation()[0], transf->Rotation()[1], transf->Rotation()[2],
					transf->Rotation()[3], transf->Rotation()[4], transf->Rotation()[5],
					transf->Rotation()[6], transf->Rotation()[7], transf->Rotation()[8]);
	...
	void ABBoxManager::ComputeABBox(VPlacedVolume const *pvol, ABBox_t *lowerc,
			ABBox_t *upperc) {
		...
		*upperc = Vector3D<Precision>(maxx + 1E-3, maxy + 1E-3, maxz + 1E-3);
		...
		Transformation3D tr(boxtranslation.x(), boxtranslation.y(),
				boxtranslation.z());
...
//                   for(int i=0;i<10000;++i)
//                   {
//                       Vector3D<Precision> p =  box.GetPointOnSurface() +
// boxtranslation;
//                       std::cerr << *lowerc << " " << * upperc << " " << p <<
// "\n";
...
void ABBoxManager::RemoveABBoxes(LogicalVolume const *lvol) {
	if (fVolToABBoxesMap[lvol->id()] != nullptr)
	delete[] fVolToABBoxesMap[lvol->id()];
}
}
}
