...
VECGEOM_CUDA_HEADER_BOTH InputIterator
...
static void SwitchFile(std::stringstream &outline, unsigned int nfiles,
                       unsigned int nfunc = 0) {
  if (nfunc) {
    outline << "} // anonymous namespace\n\n";
    outline << "VECGEOM_CUDA_HEADER_BOTH" << endl
            << "void CreateParticle" << setfill('0') << setw(4) << nfiles - 1
            << "() {" << endl
            << setfill(' ') << setw(0);
    for (unsigned int i = 0; i < nfunc; ++i)
      outline << "  InternalCreateNuclei" << setfill('0') << setw(4) << i
              << "();" << endl;
  }
  outline << "}" << endl
          << endl;
...
static void StartFile(std::stringstream &outline, unsigned int nfiles,
                      bool oneFuncPer = false) {
  ...
  */
  ...
  outline << "inline namespace VECGEOM_IMPL_NAMESPACE {" << endl
          << endl;
  outline << endl
          << "//" << setw(80) << setfill('_') << "_" << endl
          << setfill(' ') << setw(0);
  if (!oneFuncPer) {
    outline << "VECGEOM_CUDA_HEADER_BOTH" << endl
            << "void CreateNuclei" << setfill('0') << setw(4) << nfiles
            << "() {" << endl
            << setfill(' ') << setw(0);
...
VECGEOM_CUDA_HEADER_DEVICE vecgeom::map<int, vector<Nucleus *>> *fIsoListDev =
    nullptr;
vecgeom::map<int, vector<Nucleus *>> *fIsoListHost = nullptr;
VECGEOM_CUDA_HEADER_DEVICE vecgeom::map<int, vector<Nucleus *>> *
    fNatIsoListDev = nullptr;
...
std::ostream &operator<<(std::ostream &os, const Nucleus &nuc) {
  ...
  if (nuc.fNatab > 0)
    os << " Ab=" << nuc.fNatab;
  if (nuc.fIso != 0)
    os << " Level=" << nuc.fIsolevel << "MeV";
  if (nuc.fLife > 0)
    os << " Lifetime=" << nuc.fLife << "s";
  return os;
}
#endif
...
//________________________________________________________________________________________________
VECGEOM_CUDA_HEADER_BOTH
Nucleus::Nucleus(const char *name, int n, int z, int iso, double a, double dm,
                 double life, double natab, double toxa, double toxb, int ind1,
                 int ind2)
    : fN(n), fZ(z), fIso(iso), fA(a), fIsolevel(dm), fLife(life), fNatab(natab),
      fToxa(toxa), fToxb(toxb), fInd1(ind1), fInd2(ind2) {
...
void Nucleus::ReadFile(std::string infilename, bool output) {
  ...
  while (getline(infile, line)) {
    Getmat(line, n, z, iso, name, a, dm, life, da, dz, diso, br, qval, natab,
           toxa, toxb, ind1, ind2);
    if (z == 0)
      continue; // skip neutron

    int zniso = 10000 * z + 10 * n + iso;
    if (Nucleus::Nuclei().count(zniso) == 0) {
      nuc = new Nucleus(name.c_str(), n, z, iso, a, dm, life, natab, toxa, toxb,
                        ind1, ind2);
      if (output) {
        ...
        ++nnuclei;
        outline << endl
                << "   // Adding " << nuc->Name() << endl;
        outline << "   nuc = new Nucleus("
                << "\"" << name << "\""
                << "," << n << "," << z << "," << iso << "," << a << "," << dm
                << "," << life << "," << natab << "," << toxa << "," << toxb
                << "," << ind1 << "," << ind2 << ");" << endl;
        ++nlines;
      }
    }
    if (da != 0 || dz != 0 || diso != 0) {
      ...
      if (output) {
        outline << "   nuc->AddDecay(" << da << "," << dz << "," << diso << ","
                << qval << "," << br << ");" << endl;
        ++nlines;
      }
    }
  ...
  for (auto inuc = Nucleus::Nuclei().begin(); inuc != Nucleus::Nuclei().end();
       ++inuc)
  ...
  if (output) {
    ...
    outfile << "inline namespace VECGEOM_IMPL_NAMESPACE {" << endl
            << endl;
    for (unsigned int i = 0; i < nfiles; ++i) {
      outfile << "VECGEOM_CUDA_HEADER_BOTH\n";
      outfile << "void CreateNuclei" << setfill('0') << setw(4) << i << "();"
              << endl;
    }
    ...
    outfile << endl
            << "//" << setw(80) << setfill('_') << "_" << endl
            << setfill(' ') << setw(0);
    outfile << "VECGEOM_CUDA_HEADER_BOTH" << endl
            << "void Nucleus::CreateNuclei() {" << endl;
    ...
    for (unsigned int i = 0; i < nfiles; ++i)
      outfile << "  CreateNuclei" << setfill('0') << setw(4) << i << "();"
              << endl;
...
void Nucleus::Getmat(std::string line, int &n, int &z, int &iso,
  ...
  name =
      name.substr(name.find_first_not_of(" "),
                  name.find_last_not_of(" ") - name.find_first_not_of(" ") + 1);
