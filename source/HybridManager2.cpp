 ...
 */
void HybridManager2::AssignVolumesToClusters(std::vector<std::vector<int>> & clusters, SOA3D<Precision> const & centers, SOA3D<Precision> const & allvolumecenters) {
...
void HybridManager2::InitClustersWithKMeans(LogicalVolume const * lvol, Container_t & clusters, SOA3D<Precision> & centers, SOA3D<Precision> & allvolumecenters, int const numberOfIterations) {
    int numberOfClusters  = clusters.size();
        ...
        while (clusters[clusterindex].size() < clustersize) {
            int addDaughter =
                clusters[clusterindex].size() == 0
                    ...
                    : *std::min_element(daughterSet.begin(), daughterSet.end(),
                        [&](int a, int b){
                            return (allvolumecenters[a] - clusterMean).Length()
                                 < (allvolumecenters[b] - clusterMean).Length();
                        });
            ...
            clusterMean = (clusterMean * (clusters[clusterindex].size() - 1) + allvolumecenters[addDaughter]) / clusters[clusterindex].size();
        }
    }
}
