...
public:
  ...
  PlacedTrapezoid(char const *const label, LogicalVolume const *const logical_volume,
...
//   : PlacedTrapezoid("", logical_volume, transformation, boundingBox) {}
  ...
  __device__ PlacedTrapezoid(LogicalVolume const *const logical_volume, Transformation3D const *const transformation,
  ...
  {
    return static_cast<UnplacedTrapezoid const *>(GetLogicalVolume()->GetUnplacedVolume());
  }
  ...
  VECGEOM_CUDA_HEADER_BOTH
  double GetThetaCphi() const { return (double)GetUnplacedVolume()->GetTanThetaCosPhi(); }

  VECGEOM_CUDA_HEADER_BOTH
  double GetThetaSphi() const { return (double)GetUnplacedVolume()->GetTanThetaSinPhi(); }

  // This is specifically designed for Geant4 - input parameters in mm
  VECGEOM_CUDA_HEADER_BOTH
  void SetAllParameters(double dz, double theta, double phi, double dy1, double dx1, double dx2, double alp1,
...
protected:
  ...
  //   UnplacedTrapezoid const *const utrap = static_cast<UnplacedTrapezoid const
  //   *const>(logical_volume->GetUnplacedVolume());
