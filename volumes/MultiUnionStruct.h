...
struct MultiUnionStruct {
  ...
  bool ABBoxOverlap(Vector3D<double> const &amin1, Vector3D<double> const &amax1,
  	                Vector3D<double> const &amin2, Vector3D<double> const &amax2)
  {
  	// Check if two aligned boxes overlap
  	if ((amax1-amin2).Min() < -kTolerance || (amax2-amin1).Min() < -kTolerance)
  		return false;
  	return true;
  }
