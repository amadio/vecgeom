...
//This function checks whether aligned bounding boxes intersects or not.
VECGEOM_INLINE
bool IntersectionExist(Vector3D<Precision> const lowercornerFirstBox, Vector3D<Precision> const uppercornerFirstBox,
	    ...
	    if (uppercornerFirstBox.x() < lowercornerSecondBox.x()) return false; // a is left of b
	    if (lowercornerFirstBox.x() > uppercornerSecondBox.x()) return false; // a is right of b
	    if (uppercornerFirstBox.y() < lowercornerSecondBox.y()) return false; // a is left of b
	    if (lowercornerFirstBox.y() > uppercornerSecondBox.y()) return false; // a is right of b
	    if (uppercornerFirstBox.z() < lowercornerSecondBox.z()) return false; // a is left of b
	    if (lowercornerFirstBox.z() > uppercornerSecondBox.z()) return false; // a is right of b
...
//Implementation of Separation Axis Theorem for 3D
VECGEOM_INLINE
bool IntersectionExist(Vector3D<Precision> const lowercornerFirstBox, Vector3D<Precision> const uppercornerFirstBox,
		Vector3D<Precision> const lowercornerSecondBox, Vector3D<Precision> const uppercornerSecondBox,
	...
	//Vector3D<Precision> pA(2.,1.5,2.5), pB(6.,1.5,2.5); //Centre of two oriented bounding boxes A, B //HardCoded for testing purpose only
	///Vector3D<Precision> pA(lowercornerFirstBox.x()+halfAx , lowercornerFirstBox.y()+halfAy, lowercornerFirstBox.z()+halfAz),
	//		pB(lowercornerSecondBox.x()+halfBx , lowercornerSecondBox.y()+halfBy, lowercornerSecondBox.z()+halfBz);
	//Vector3D<Precision> axesA=pA.Unit(), axesB=pB.Unit();
	//Vector3D<Precision> pA(1,1,1), pB(1,1,1); //Centre of two oriented bounding boxes A, B //HardCoded for testing purpose only

	//Vector3D<Precision> pA(lowercornerFirstBox.x()+halfAx , lowercornerFirstBox.y()+halfAy, lowercornerFirstBox.z()+halfAz),
		//		pB(lowercornerSecondBox.x()+halfBx , lowercornerSecondBox.y()+halfBy, lowercornerSecondBox.z()+halfBz);
	//Vector3D<Precision> T = pB-pA;
	/*
	Vector3D<Precision> Ax(1.,0.,0.);
	Vector3D<Precision> Ay(0.,1.,0.);
	Vector3D<Precision> Az(0.,0.,1.);

	Vector3D<Precision> Bx(1.,0.,0.);
	Vector3D<Precision> By(0.,1.,0.);
	Vector3D<Precision> Bz(0.,0.,1.);
	*/
	...
	//Case 1:
	//L = Ax
	//std::cout<<" 1 : "<<std::fabs(T.Dot(Ax))<<" :: 2 : "<<(halfAx + std::fabs(halfBx*Ax.Dot(Bx)) + std::fabs(halfBy*Ax.Dot(By)) + std::fabs(halfBz*Ax.Dot(Bz)) )<<std::endl;
	if(std::fabs(T.Dot(Ax)) > (halfAx + std::fabs(halfBx*Ax.Dot(Bx)) + std::fabs(halfBy*Ax.Dot(By)) + std::fabs(halfBz*Ax.Dot(Bz)) ) )
	{	//std::cout<<"---- Case-1 matches ----"<<std::endl;
	//std::cout<<"---- Separation Axis found, Intersection Does not Exist ----"<<std::endl;
		return false;
	}

	//Case 2:
	//L = Ay
	if(std::fabs(T.Dot(Ay)) > (halfAy + std::fabs(halfBx*Ay.Dot(Bx)) + std::fabs(halfBy*Ay.Dot(By)) + std::fabs(halfBz*Ay.Dot(Bz)) ) )
	{	//std::cout<<"---- Case-2 matches ----"<<std::endl;
	//std::cout<<"---- Separation Axis found, Intersection Does not Exist ----"<<std::endl;
		return false;
	}
	...
	//Case 3:
	//L = Az
	if(std::fabs(T.Dot(Az)) > (halfAz + std::fabs(halfBx*Az.Dot(Bx)) + std::fabs(halfBy*Az.Dot(By)) + std::fabs(halfBz*Az.Dot(Bz)) ) )
	{
	//std::cout<<"---- Case-3 matches ----"<<std::endl;
	//std::cout<<"---- Separation Axis found, Intersection Does not Exist ----"<<std::endl;
	return false;
	}
	...
	//Case 4:
	//L = Bx
	if(std::fabs(T.Dot(Bx)) > (halfBx + std::fabs(halfAx*Ax.Dot(Bx)) + std::fabs(halfAy*Ay.Dot(Bx)) + std::fabs(halfAz*Az.Dot(Bx)) ) )
	{
	//std::cout<<"---- Case-4 matches ----"<<std::endl;
	//std::cout<<"---- Separation Axis found, Intersection Does not Exist ----"<<std::endl;
	return false;
	}
	...
	//Case 5:
	//L = By
	if(std::fabs(T.Dot(By)) > (halfBy + std::fabs(halfAx*Ax.Dot(By)) + std::fabs(halfAy*Ay.Dot(By)) + std::fabs(halfAz*Az.Dot(By)) ) )
	{
	//std::cout<<"---- Case-5 matches ----"<<std::endl;
	//std::cout<<"---- Separation Axis found, Intersection Does not Exist ----"<<std::endl;
	return false;
	}
	...
	//Case 6:
	//L = Bz
	if(std::fabs(T.Dot(Bz)) > (halfBz + std::fabs(halfAx*Ax.Dot(Bz)) + std::fabs(halfAy*Ay.Dot(Bz)) + std::fabs(halfAz*Az.Dot(Bz)) ) )
	{
	//std::cout<<"---- Case-6 matches ----"<<std::endl;
	//std::cout<<"---- Separation Axis found, Intersection Does not Exist ----"<<std::endl;
	return false;
	}
	...
	if( ( std::fabs(T.Dot(Az)*Ay.Dot(Bx) - T.Dot(Ay)*Az.Dot(Bx)) ) >
							( std::fabs(halfAy*Az.Dot(Bx)) + std::fabs(halfAz*Ay.Dot(Bx)) + std::fabs(halfBy*Ax.Dot(Bz)) + std::fabs(halfBz*Ax.Dot(By)) ) ){
		return false;
	}
	...
	if( ( std::fabs(T.Dot(Az)*Ay.Dot(By) - T.Dot(Ay)*Az.Dot(By)) ) >
						( std::fabs(halfAy*Az.Dot(By)) + std::fabs(halfAz*Ay.Dot(By)) + std::fabs(halfBx*Ax.Dot(Bz)) + std::fabs(halfBz*Ax.Dot(Bx)) ) ){
		return false;
	}
	...
	if( ( std::fabs(T.Dot(Az)*Ay.Dot(Bz) - T.Dot(Ay)*Az.Dot(Bz)) ) >
							( std::fabs(halfAy*Az.Dot(Bz)) + std::fabs(halfAz*Ay.Dot(Bz)) + std::fabs(halfBx*Ax.Dot(By)) + std::fabs(halfBy*Ax.Dot(Bx)) ) ){
		return false;
	}
	...
	if( ( std::fabs(T.Dot(Ax)*Az.Dot(Bx) - T.Dot(Az)*Ax.Dot(Bx)) ) >
							( std::fabs(halfAx*Az.Dot(Bx)) + std::fabs(halfAz*Ax.Dot(Bx)) + std::fabs(halfBy*Ay.Dot(Bz)) + std::fabs(halfBz*Ay.Dot(By)) ) ){
		return false;
	}
	...
	if( ( std::fabs(T.Dot(Ax)*Az.Dot(By) - T.Dot(Az)*Ax.Dot(By)) ) >
							( std::fabs(halfAx*Az.Dot(By)) + std::fabs(halfAz*Ax.Dot(By)) + std::fabs(halfBx*Ay.Dot(Bz)) + std::fabs(halfBz*Ay.Dot(Bx)) ) ){
		return false;
	}
	...
	if( ( std::fabs(T.Dot(Ax)*Az.Dot(Bz) - T.Dot(Az)*Ax.Dot(Bz)) ) >
							( std::fabs(halfAx*Az.Dot(Bz)) + std::fabs(halfAz*Ax.Dot(Bz)) + std::fabs(halfBx*Ay.Dot(By)) + std::fabs(halfBy*Ay.Dot(Bx)) ) ){
		return false;
	}
	...
	if( ( std::fabs(T.Dot(Ay)*Ax.Dot(Bx) - T.Dot(Ax)*Ay.Dot(Bx)) ) >
							( std::fabs(halfAx*Ay.Dot(Bx)) + std::fabs(halfAy*Ax.Dot(Bx)) + std::fabs(halfBy*Az.Dot(Bz)) + std::fabs(halfBz*Az.Dot(By)) ) ){
		return false;
	}
	...
	if( ( std::fabs(T.Dot(Ay)*Ax.Dot(By) - T.Dot(Ax)*Ay.Dot(By)) ) >
							( std::fabs(halfAx*Ay.Dot(By)) + std::fabs(halfAy*Ax.Dot(By)) + std::fabs(halfBx*Az.Dot(Bz)) + std::fabs(halfBz*Az.Dot(Bx)) ) ){
		return false;
	}
	...
	if( ( std::fabs(T.Dot(Ay)*Ax.Dot(Bz) - T.Dot(Ax)*Ay.Dot(Bz)) ) >
							( std::fabs(halfAx*Ay.Dot(Bz)) + std::fabs(halfAy*Ax.Dot(Bz)) + std::fabs(halfBx*Az.Dot(By)) + std::fabs(halfBy*Az.Dot(Bx)) ) ){
		return false;
	}
