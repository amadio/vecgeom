...
public:
  ...
  double DistanceToOut(Vector3D<double> const &position,
    ...
    if (output < kHalfTolerance)
      output = 0.;
    return output;
  }

  EnumInside Inside (const UVector3 &aPoint) const override {
  ...
  double SafetyFromOutside(Vector3D<double> const &point,
                           bool accurate = false) const override {
  ...
  double SafetyFromInside(Vector3D<double> const &point,
                          bool accurate = false) const override {
    (void)accurate; // fix 'unused variable' warning
  ...
  double DistanceToOut(Vector3D<double> const &point,
                       Vector3D<double> const &direction,
                       Vector3D<double> &normal,
                       bool &convex,
                       double stepMax = kInfinity) const override {
  ...
  double DistanceToIn(Vector3D<double> const &position,
                      Vector3D<double> const &direction,
  ...
  void Extent( UVector3 &aMin, UVector3 &aMax ) const override {
 ...
 // }
  ...
  UVector3 GetPointOnSurface() const override {
    return UnplacedVolume_t::GetPointOnSurface();
  }
