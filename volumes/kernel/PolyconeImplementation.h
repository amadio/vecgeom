...
// }
                            ...
                            typename Backend::precision_v const &stepMax, typename Backend::precision_v &distance)
  {
    ...
    /*
    int indexOut = polycone.GetSectionIndex(point.z());
    if(indexOut < 0){
    distance = -1;
    return;
    }
   const PolyconeSection &sectionO = polycone.GetSection(indexOut);

 typename Backend::int_v insideO;
      ConeImplementation<translation::kIdentity, rotation::kIdentity, ConeTypes::UniversalCone>::Inside<Backend>(
          *sectionO.fSolid, Transformation3D(), point - Vector3D<Precision>(0, 0, sectionO.fShift), insideO);
      if (insideO == EInside::kOutside){
      distance=-1;
      return;
     }
    */
    ...
    do {
...
#ifdef Temp
        typename Backend::int_v inside2;
        const PolyconeSection &sectionTemp = polycone.GetSection(index + 1);
        ConeImplementation<translation::kIdentity, rotation::kIdentity, ConeTypes::UniversalCone>::Inside<Backend>(
            *section.fSolid, Transformation3D(), pn, inside2);
        if (verbose) std::cout << "Location w.r.t next section : " << inside2 << std::endl;

        bool insidet;
        Vector3D<Precision> localp;
        if (verbose) std::cout << "- --- Trying out same thing using CONTAINS ---- " << std::endl;
        ConeImplementation<translation::kIdentity, rotation::kIdentity, ConeTypes::UniversalCone>::Contains<Backend>(
            *sectionTemp.fSolid, Transformation3D(), pn, localp, insidet);
        if (verbose) std::cout << "Location from Contains function : " << insidet << std::endl;

#endif
        if (verbose) std::cout << "Location w.r.t  first legal section : " << inside << std::endl;
        if (inside == EInside::kOutside) {
          if (verbose) std::cout << "---- this should NOT come ---- " << std::endl;
          break;
        }
      } else
        pn.z() -= section.fShift;
      ...
      if (verbose) std::cout << "Distance from section : " << index << " : " << dist << std::endl;
      ...
      if (std::fabs(dist) < 0.5 * kTolerance) {
        ...
        if (verbose) std::cout << "Point : " << point << " : PTE : " << pte << std::endl;
        ...
        // std::cout<<"INSIDE-22 : "<< inside22 << std::endl;
        //
        //  typedef typename Backend::bool_v Bool_t;
        //  Bool_t compleltelyinside(false),completelyoutside(false);
        // ConeImplementation<translation::kIdentity, rotation::kIdentity,
        // ConeTypes::UniversalCone>::GenericKernelForContainsAndInside<Backend, true>( *section1.fSolid, pte,
        // compleltelyinside, completelyoutside);
        // if (!inside1) {
        if (inside22 == 3 || (increment == 0)) {
          // if(completelyoutside || (increment == 0)){
          // if(!inside1){
          if (verbose) {
            std::cout << "Distance calculation from section : " << index << " : " << dist << std::endl;
            std::cout << "--- Executing Break ---- " << std::endl;
          }
          break;
        }
      } // end if surface case
 ...
 */
    ...
    safety                                 = minSafety;
    if (safety < kTolerance * 100.) safety = 0.;
    return;
  ...
  // New Definition that uses new Cone
  template <class Backend>
  VECGEOM_FORCE_INLINE
  VECGEOM_CUDA_HEADER_BOTH
  static void SafetyToOut(UnplacedPolycone const &polycone, Vector3D<typename Backend::precision_v> const &point,
                          typename Backend::precision_v &safety)
  {
    ...
    Vector3D<typename Backend::precision_v> p = point - Vector3D<Precision>(0, 0, sec.fShift);
    ConeImplementation<translation::kIdentity, rotation::kIdentity,
                       ConeTypes::UniversalCone>::SafetyToOutKernel<Backend>(*sec.fSolid, p, safety);
    if (compIn && safety == 0.) safety = kInfLength;
    Precision minSafety                = safety;
    Precision zbase                    = polycone.fZs[index + 1];
    for (int i = index + 1; i < polycone.GetNSections(); ++i) {
      Precision dz = polycone.fZs[i] - zbase;
      ...
      if (compIn && safety == 0.) // implies we are on segment-separating plane
        safety = kConeTolerance;
      // safety = kInfLength;
