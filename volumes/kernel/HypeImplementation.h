...
static void PrintType() {
   printf("SpecializedHype<%i, %i>", transCodeT, rotCodeT);
        ...
        VECGEOM_INLINE
        static void
        Normal(UnplacedHype const &unplaced, Vector3D<typename Backend::precision_v> const &point,
        ...
        VECGEOM_INLINE
        static void
        NormalKernel(UnplacedHype const &unplaced, Vector3D<typename Backend::precision_v> const &point,
                     Vector3D<typename Backend::precision_v> &normal, typename Backend::bool_v &valid);

        template <typename Backend, bool ForInnerRad>
        VECGEOM_CUDA_HEADER_BOTH
        VECGEOM_INLINE static typename Backend::precision_v
        RadiusHypeSq(UnplacedHype const &unplaced, typename Backend::precision_v z);
        ...
        VECGEOM_CUDA_HEADER_BOTH
        VECGEOM_INLINE
        static typename Backend::bool_v IsPointMovingInsideOuterSurface(UnplacedHype const &unplaced,
                                        Vector3D<typename Backend::precision_v> const &point,
                                        Vector3D<typename Backend::precision_v> const &direction);
        ...
        VECGEOM_CUDA_HEADER_BOTH
        VECGEOM_INLINE
        static typename Backend::bool_v IsPointMovingInsideInnerSurface(UnplacedHype const &unplaced,
                                        Vector3D<typename Backend::precision_v> const &point,
                                        Vector3D<typename Backend::precision_v> const &direction);
        ...
        VECGEOM_CUDA_HEADER_BOTH
        VECGEOM_INLINE
        static typename Backend::bool_v IsPointMovingOutsideOuterSurface(UnplacedHype const &unplaced,
                                         Vector3D<typename Backend::precision_v> const &point,
                                         Vector3D<typename Backend::precision_v> const &direction);
        ...
        VECGEOM_CUDA_HEADER_BOTH
        VECGEOM_INLINE
        static typename Backend::bool_v IsPointMovingOutsideInnerSurface(UnplacedHype const &unplaced,
                                         Vector3D<typename Backend::precision_v> const &point,
                                         Vector3D<typename Backend::precision_v> const &direction);
        ...
        VECGEOM_CUDA_HEADER_BOTH
        VECGEOM_INLINE
        static typename Backend::bool_v IsPointMovingInsideFromZPlane(UnplacedHype const &unplaced,
                                        Vector3D<typename Backend::precision_v> const &point,
                                        Vector3D<typename Backend::precision_v> const &direction);
        ...
        VECGEOM_CUDA_HEADER_BOTH
        VECGEOM_INLINE
        static typename Backend::bool_v IsPointMovingOutsideFromZPlane(UnplacedHype const &unplaced,
                                        Vector3D<typename Backend::precision_v> const &point,
                                        Vector3D<typename Backend::precision_v> const &direction);
...
template <TranslationCode transCodeT, RotationCode rotCodeT>
    ...
    HypeImplementation<transCodeT, rotCodeT>::IsPointMovingInsideFromZPlane(
        UnplacedHype const &unplaced, Vector3D<typename Backend::precision_v> const &point,
        Vector3D<typename Backend::precision_v> const &direction) {
    ...
    typename Backend::bool_v HypeImplementation<transCodeT, rotCodeT>::IsPointOnSurfaceAndMovingOutside(
      ...
      out |=
          (!done && !zSurf && outerHypeSurf && IsPointMovingOutsideOuterSurface<Backend>(unplaced, point, direction));
      ...
      if (unplaced.InnerSurfaceExists()) {
        ...
        out |=
            (!done && !zSurf && innerHypeSurf && IsPointMovingOutsideInnerSurface<Backend>(unplaced, point, direction));
	...
	typename Backend::bool_v HypeImplementation<transCodeT, rotCodeT>::IsPointOnInnerSurfaceAndMovingOutside(
      ...
      if (unplaced.InnerSurfaceExists()) {
        ...
        // MaskedAssign(innerHypeSurf,Bool_t(IsPointMovingOutsideInnerSurface<Backend>(unplaced,point,direction))
        // ,&out);
    ...
    typename Backend::bool_v HypeImplementation<transCodeT, rotCodeT>::IsPointOnSurfaceAndMovingInside(
      ...
      if (unplaced.InnerSurfaceExists()) {
        ...
        innerHypeSurf |= (!zSurf && !outerHypeSurf && (Abs((radI2) - (rho2)) < unplaced.GetInnerRadToleranceLevel()));
        in |=
            (!done && !zSurf && innerHypeSurf && IsPointMovingInsideInnerSurface<Backend>(unplaced, point, direction));
        done |= (!zSurf && innerHypeSurf);
        if (IsFull(done))
          return in;
      }
    ...
    typename Backend::precision_v HypeImplementation<transCodeT, rotCodeT>::RadiusHypeSq(
                                  UnplacedHype const &unplaced, typename Backend::precision_v z) {
    ...
    template <typename Backend>
    VECGEOM_CUDA_HEADER_BOTH
    typename Backend::bool_v HypeImplementation<transCodeT, rotCodeT>::IsCompletelyInside( UnplacedHype const &unplaced,
                                                                                           Vector3D<typename Backend::precision_v> const &point) {
      ...
      Float_t r2 = point.Perp2(); //(point.x() * point.x() + point.y() * point.y());
    ...
    template <typename Backend>
    VECGEOM_CUDA_HEADER_BOTH
    typename Backend::bool_v HypeImplementation<transCodeT, rotCodeT>::IsCompletelyOutside( UnplacedHype const &unplaced,
                                                                                           Vector3D<typename Backend::precision_v> const &point) {
...
template <TranslationCode transCodeT, RotationCode rotCodeT>
    ...
    void HypeImplementation<transCodeT, rotCodeT>::DistanceToInKernel(
                                                                     ...
                                                                     typename Backend::precision_v const &stepMax,
                                                                     typename Backend::precision_v &distance) {
        //std::cout<<"----- Entered DistanceToOutKernel -------"<<std::endl;
        typedef typename Backend::precision_v Float_t;
        typedef typename Backend::bool_v      Bool_t;
        ...
        cond = (hittingOuterSurfaceFromOutsideZRange || hittingOuterSurfaceFromWithinZRange || (IsPointOnOuterSurfaceAndMovingOutside<Backend>(unplaced,point,direction)))
        ...
        cond = (hittingInnerSurfaceFromOutsideZRange || hittingInnerSurfaceFromWithinZRange || (IsPointOnInnerSurfaceAndMovingOutside<Backend>(unplaced,point,direction)))
        ...
        //std::cout<<"Distance from DistanceToInKernel : "<<distance<<std::endl;
        // Bool_t surfaceCond = IsPointOnSurfaceAndMovingInside<Backend>(unplaced,point,direction);
        // MaskedAssign(!done && surfaceCond , 0. , &distance);//,isSurfacePoint,inner,outer,zSurf),0., &distance);
        // done |= surfaceCond;
        // if(IsFull(done)) return;
    ...
    template <class Backend, bool ForDistToIn>
    VECGEOM_CUDA_HEADER_BOTH typename Backend::bool_v
    HypeImplementation<transCodeT, rotCodeT>::GetPointOfIntersectionWithZPlane(
        UnplacedHype const &unplaced, Vector3D<typename Backend::precision_v> const &point,
    ...
    template <class Backend, bool ForDistToIn>
    VECGEOM_CUDA_HEADER_BOTH typename Backend::bool_v
    HypeImplementation<transCodeT, rotCodeT>::GetPointOfIntersectionWithInnerHyperbolicSurface(
        UnplacedHype const &unplaced, Vector3D<typename Backend::precision_v> const &point,
        Vector3D<typename Backend::precision_v> const &direction,
    ...
    template <class Backend, bool ForDistToIn>
    VECGEOM_CUDA_HEADER_BOTH typename Backend::bool_v
    HypeImplementation<transCodeT, rotCodeT>::GetPointOfIntersectionWithOuterHyperbolicSurface(
        UnplacedHype const &unplaced, Vector3D<typename Backend::precision_v> const &point,
        Vector3D<typename Backend::precision_v> const &direction,
        typename Backend::precision_v &dist) {
    ...
    void HypeImplementation<transCodeT, rotCodeT>::DistanceToOutKernel(
        ...
        //std::cout<<"DistFrom Outer Hype Surface : "<<dist<<"  :: Distance : "<<distance<< std::endl;
        ...
        //std::cout<<"DistFrom Inner Hype Surface : "<<dist<<std::endl;
        //MaskedAssign(!done && intersectionExist && dist<distance, dist , &distance);
        ...
        // MaskedAssign(IsCompletelyOutside<Backend>(unplaced,point), -1., &distance);

       // std::cout<<"Final Distance from DistanceToOutKernel : "<<distance<<std::endl;
    ...
    MaskedAssign(!done && compIn, -1., &safety);
    done |= compIn;
    if(IsFull(done)) return;
    ...
    MaskedAssign(!done && (r2 < ( (innerRadius*innerRadius + tanInnerStereo2*absZ*absZ )-kHalfTolerance) ) && (absZ > 0.) && (absZ < unplaced.GetDz()) ,
