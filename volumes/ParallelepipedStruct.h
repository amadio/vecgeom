...
struct ParallelepipedStruct {
  ...
  ParallelepipedStruct(Vector3D<T> const &dim, const T alpha, const T theta, const T phi) 
    : fDimensions(dim), fAlpha(0), fTheta(0), fPhi(0), fCtx(0), fCty(0), fTanAlpha(0),
      fTanThetaSinPhi(0), fTanThetaCosPhi(0) {
    SetAlpha(alpha);
  ...
  ParallelepipedStruct(const T x, const T y, const T z, const T alpha, const T theta, const T phi)
    : fDimensions(x, y, z), fAlpha(0), fTheta(0), fPhi(0), fCtx(0), fCty(0), fTanAlpha(0),
      fTanThetaSinPhi(0), fTanThetaCosPhi(0) {
    SetAlpha(alpha);
    SetThetaAndPhi(theta, phi);
  }  
  ...
  void ComputeNormals()
  {
    Vector3D<T> v(sin(kDegToRad * fTheta) * cos(kDegToRad * fPhi),
