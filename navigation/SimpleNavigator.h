...
public:
   ...
   * input: pointer to starting placed volume in the hierarchy and a global point, we also give an indication if we call
   * this from top
  ...
  inline void FindNextBoundaryAndStep(Vector3D<Precision> const & /* global point */,
                                      ...
                                      Precision const & /* proposed physical step */, Precision & /*step*/
                                      ) const;
   ...
   * Note that the user has to provide a couple of workspace memories; This is the easiest way to make the navigator
   * fully
   * threadsafe
   */
  template <typename Container3D>
  ...
  void FindNextBoundaryAndStep(Container3D const & /*global point*/, Container3D const & /*global dirs*/,
                               ...
                               Precision const * /* pSteps -- proposed steps */, Precision * /* safeties */,
   ...
   */
  void InspectEnvironmentForPointAndDirection(Vector3D<Precision> const & /* global point */,
   ...
   */
  void InspectSafetyForPoint(Vector3D<Precision> const & /* global point */, NavigationState const & /* current state */
                             ) const;

#ifdef VECGEOM_ROOT
  /**
   * function to dump navigation situation in order to be able to replay certain FindNextBoundaryAndStep() function
   * calls
   * in a separate program
   */
  void CreateDebugDump(Vector3D<Precision> const & /* global_point */, Vector3D<Precision> const & /* global_point */,
                       NavigationState const & /* current state */, double const pstep) const;
#endif
  ...
  if (candvolume) {
    ...
    while (godeeper && daughters->size() > 0) {
      ...
      for (int boxgroupid = 0; boxgroupid < size; ++boxgroupid) {
        ...
        Bool_v inBox;
        ABBoxImplementation::ABBoxContainsKernel(alignedbboxes[2 * boxgroupid], alignedbboxes[2 * boxgroupid + 1], tmp,
                                                 inBox);
...
VPlacedVolume const *SimpleNavigator::RelocatePointFromPath(Vector3D<Precision> const &localpoint,
...
//      std::cout << " counter is " << counter << "\n";
  ...
  } else // hitting mother
  {
    ...
    // LocateLocalPointFromPath_Relative_Iterative( newpointafterboundary, newpointafterboundaryinnewframe, outpath,
    // globalm );
  ...
  if (newstate.Top() != vecgeomcmpnext || currentstate.Top() != vecgeomcmpcur) {
    ...
    std::cout << "  ROOT current volume " << (orignode ? orignode->GetVolume()->GetName() : " NULL ") << "\n";
    std::cout << "  VecGeom current volume " << (currentstate.Top() ? currentstate.Top()->GetLabel() : " NULL ")
              << "\n";
    ...
    G4ThreeVector g4p(10 * globalpoint.x(), 10 * globalpoint.y(), 10 * globalpoint.z()),
        g4d(globaldir.x(), globaldir.y(), globaldir.z());
    ...
    std::cout << "  G4 next volume " << ((g4nextvol != NULL) ? g4nextvol->GetName() : "NULL") << "\n";
#endif
...
Precision SimpleNavigator::GetSafety(Vector3D<Precision> const &globalpoint, NavigationState const &currentstate) const
{
...
template <typename Container3D>
void SimpleNavigator::GetSafeties(Container3D const &globalpoints, NavigationState **states,
                                  Container3D &workspaceforlocalpoints, Precision *safeties) const
{
...
#define CALCSAFETY
template <typename Container3D>
void SimpleNavigator::FindNextBoundaryAndStep(Container3D const &globalpoints, Container3D const &globaldirs,
                                              Container3D &localpoints, Container3D &localdirs,
                                              NavigationState **currentstates, NavigationState **newstates,
                                              Precision const *pSteps, Precision *safeties, Precision *distances,
                                              int *nextnodeworkspace) const
{
  ...
  currentvolume->DistanceToOut(localpoints, localdirs, pSteps, distances, nextnodeworkspace);
  ...
  for (size_t daughterindex = 0; daughterindex < daughters->size(); ++daughterindex) {
    ...
    daughter->DistanceToInMinimize(localpoints, localdirs, daughterindex, distances, nextnodeworkspace);
  }
