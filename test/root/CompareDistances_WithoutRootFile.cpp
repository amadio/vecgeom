...
int main(int argc, char *argv[])
{
  ...
  TGeoVolume *foundvolume = gGeoManager->MakeTubs("Tube", med, 1., 6., 2., 0., dphi);//0.6);
...
*/
  ...
  if (rootback) {
    ...
    std::cout << "ROOTBACKCONV DI " << rootback->DistFromOutside(&Vector3D<double>(point)[0], &Vector3D<double>(dir)[0])
              << "\n";
    std::cout << "ROOTBACKCONV DO " << rootback->DistFromInside(&Vector3D<double>(point)[0], &Vector3D<double>(dir)[0])
              << "\n";
