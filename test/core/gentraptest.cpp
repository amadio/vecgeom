...
void createGenTrap() {
  // no twist
  Precision verticesx1[8] = {-3, -3, 3, 3, -3, -3, 3, 3};
  Precision verticesy1[8] = {-3, 3, 3, -3, -3, 3, 3, -3};

  UnplacedGenTrap trapUnplaced1(verticesx1, verticesy1, 10);
  my_assert(trapUnplaced1.ComputeIsTwisted() == false, "twisted");

  Precision verticesx2[8] = {-3, -3, 3, 3, -0.5, -2, 2, 2};
  Precision verticesy2[8] = {-3, 3, 3, -3, -2, 2, 2, -2};

  UnplacedGenTrap trapUnplaced2(verticesx2, verticesy2, 10);
  ...
  vecgeom::GenTrapImplementation<0, 0>::GenericKernelForContainsAndInside<kScalar, false>(
      trapUnplaced2, localPoint, completelyInside, completelyOutside);
  //  my_assert( completelyInside == true, "inside1" );
  my_assert(completelyOutside == false, "outside1");

  Vector3D<Precision> localPoint2(-10., 0., 0.);
  vecgeom::GenTrapImplementation<0, 0>::GenericKernelForContainsAndInside<kScalar, false>(
      trapUnplaced2, localPoint2, completelyInside, completelyOutside);
  ...
  Precision verticesx3[8] = {-3, -3, 3, 3, -3, -3, 3, 3};
  Precision verticesy3[8] = {-2.5, 3, 3, -3, -3, 3, 2, -3};

  UnplacedGenTrap trapUnplaced3(verticesx3, verticesy3, 10);
  vecgeom::GenTrapImplementation<vecgeom::translation::kGeneric, vecgeom::rotation::kIdentity>::DistanceToIn<kScalar>(
      trapUnplaced3, Transformation3D(5., 5., 5.), Vec_t(-9.441650, 0.902802, 10.916641),
      Vec_t(0.954990, 0.247627, -0.163324), step, d);
  assert(std::abs(d - 11.9809) < 1E-4);

  vecgeom::GenTrapImplementation<vecgeom::translation::kGeneric, vecgeom::rotation::kIdentity>::DistanceToIn<kScalar>(
      trapUnplaced3, Transformation3D(5., 5., 5.), Vec_t(4.546874, 8.865740, 8.098616),
      Vec_t(-0.382036, -0.561047, -0.734353), step, d);
  //    std::cerr << "final " << d << "\n"; //  1.86284
  assert(std::abs(d - 1.86284) < 1.E-4);

  // should have a macro for this
  vecgeom::GenTrapImplementation<vecgeom::translation::kGeneric, vecgeom::rotation::kIdentity>::DistanceToIn<kScalar>(
      trapUnplaced3, Transformation3D(5., 5., 5.), Vec_t(-3.642379, -7.785746, 18.814608),
      Vec_t(0.390819, 0.361792, -0.846385), step, d);
  //    std::cerr << "final " << d << "\n"; //  27.2668
  assert(std::abs(d - 27.2668) < 1.E-4);

  vecgeom::GenTrapImplementation<vecgeom::translation::kGeneric, vecgeom::rotation::kIdentity>::DistanceToIn<kScalar>(
      trapUnplaced3, Transformation3D(5., 5., 5.), Vec_t(-7.569044, -4.983397, 4.661511),
      Vec_t(0.838801, 0.531342, -0.118692), step, d);
  std::cerr << "final " << d << "\n"; //  13.5376
  assert(std::abs(d - 13.5376) < 1.E-4);

  // see if root result is actually good
  Vec_t localp;
  vecgeom::GenTrapImplementation<vecgeom::translation::kGeneric, vecgeom::rotation::kIdentity>::Contains<kScalar>(
      trapUnplaced3, Transformation3D(5., 5., 5.),
      Vec_t(-7.569044, -4.983397, 4.661511) + (13.5376 + 1.E-4) * Vec_t(0.838801, 0.531342, -0.118692), localp,
      completelyInside);
  assert(completelyInside == true);

  vecgeom::GenTrapImplementation<vecgeom::translation::kGeneric, vecgeom::rotation::kIdentity>::DistanceToIn<kScalar>(
      trapUnplaced3, Transformation3D(5., 5., 5.), Vec_t(2.919067, 8.152522, 15.885466),
      Vec_t(-0.173286, -0.788227, -0.590482), step, d);
  std::cerr << "final " << d << "\n"; //  13.5376
  ...
  Vc_t vcd;
  vecgeom::GenTrapImplementation<vecgeom::translation::kGeneric, vecgeom::rotation::kIdentity>::DistanceToIn<kVc>(
      trapUnplaced3, Transformation3D(5., 5., 5.), VecVc_t(Vc_t(2.919067), Vc_t(8.152522), Vc_t(15.885466)),
      VecVc_t(Vc_t(-0.173286), Vc_t(-0.788227), Vc_t(-0.590482)), Vc_t(100.), vcd);
  std::cerr << "final " << vcd << "\n"; //  13.5376
                                        // assert( std::abs(d-1.49956 )<1.E-4);

  vecgeom::GenTrapImplementation<vecgeom::translation::kGeneric, vecgeom::rotation::kIdentity>::DistanceToIn<kScalar>(
      trapUnplaced3, Transformation3D(5., 5., 5.), Vec_t(5.112479, 7.682771, 12.854574),
      Vec_t(-0.796774, -0.503014, -0.334855), step, d);
  std::cerr << "final " << d << "\n";
