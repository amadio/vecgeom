...
int ShapeTester::ShapeDistances()
{
  ...
  for ( i = 0; i < fMaxPointsInside; i++)
  {
    ...
    if( ( fVolumeUSolids->Inside(pointOut-dir*DistanceIn)!=vecgeom::EInside::kOutside )&&(maxDifIn < DistanceIn ))
    { maxDifIn = DistanceIn;}
...
int ShapeTester::TestFarAwayPoint()
{
  ...
  for ( int j=0; j<fMaxPointsInside; j++)
  {
    ...
    if(fVolumeUSolids->DistanceToIn(point,vec) < UUtils::kInfinity)continue;
    point1= point;
  ...
  if(fVerbose)
  {
    std::cout<<"% TestFarAwayPoints:: number of fPoints with big difference (( DistanceToIn- Dist) ) >  tolerance ="<<icount<<std::endl;
    std::cout <<"%  Maxdif = "<<difMax<<" from MaxDist="<<maxDistIn<<" Number of fPoints missing Solid (DistanceToIn = Infinity) = "<<icount1<<std::endl;
  }
  std::cout<<"% "<<std::endl;
...
int ShapeTester::TestSurfacePoint()
{
  ...
  if(fVerbose){
    std::cout<<"% TestSurfacePoints GetPointOnSurface() for Solid  "<<fVolumeUSolids->GetName()<<" had "<<icount<<" errors"<<std::endl;
    std::cout<<"% TestSurfacePoints both  DistanceToIN and DistanceToOut ==0 for "<<fVolumeUSolids->GetName()<<" had "<<icount1<<" errors"<<std::endl;
      ...
      if(inside ==  vecgeom::EInside::kOutside)
      {
        ...
        for(int j = 0; j < 1000; j++ )
        {
          ...
          if(fVolumeUSolids->Inside(pointIn) ==  vecgeom::EInside::kInside)
          {
            ...
            double distIn1   = fVolumeUSolids->DistanceToIn(pointIn,vec);
            iWrongSideIn++;
            ...
            // for VecGeom, distIn1 should be -1 for fPoints inside
            ...
            double safFromOut = fVolumeUSolids->SafetyFromOutside(pointIn);
...
int  ShapeTester::ShapeSafetyFromInside(int max)
{
  ...
  if( max > fMaxPoints )max=fMaxPoints;
  for (int i = 0; i < max; i++)
  {
    GetVectorUSolids(point, fPoints, i);
    double res = fVolumeUSolids->SafetyFromInside(point);
  ...
  if(fVerbose){
    std::cout<<"% "<<std::endl;
    std::cout<<"% ShapeSafetyFromInside ::  number of Points Outside Safety="<<count<<" number of fPoints with  distance smaller that safety="<<count1<<std::endl;
    std::cout<<"% "<<std::endl;
  }
#ifdef VECGEOM_ROOT
  //visualisation
...
int ShapeTester::ShapeSafetyFromOutside(int max)
{
  ...
  if(fVerbose){
    std::cout<<"% "<<std::endl;
    std::cout<<"% TestShapeSafetyFromOutside::  number of fPoints Inside Safety Sphere ="<<count<<" number of fPoints with Distance smaller that Safety="<<count1<<std::endl;
    std::cout<<"% "<<std::endl;
  }
#ifdef VECGEOM_ROOT
  //visualisation
...
int ShapeTester::TestXRayProfile()
{
  ...
  std::cout<<"% \n"<<std::endl;
  if(fGNumberOfScans==1) {errCode+= Integration(0,45,200,true);}//1-theta,2-phi
  else{  errCode+= XRayProfile(0,fGNumberOfScans,1000);}
...
int ShapeTester::XRayProfile(double theta, int nphi, int ngrid, bool useeps)
{
  ...
  TH1F *hxprofile = new TH1F("xprof", Form("X-ray capacity profile of shape %s for theta=%g degrees", fVolumeUSolids->GetName().c_str(), theta),
                             nphi, 0, 360);
...
// integrate shape capacity by sampling rays
  ...
  if (graphics) {
    ...
    xprof = new TH2F("x-ray",Form("X-ray profile from theta=%g phi=%g of shape %s", theta, phi, fVolumeUSolids->GetName().c_str()),
  ...
  if((fVerbose) && (graphics)){
    printf("th=%g phi=%g: analytical: %f    --------   sampled: %f +/- %f\n", theta, phi, fGCapacityAnalytical ,fGCapacitySampled , fGCapacityError);
    printf("Hit ratio: %f\n", double(nhit)/ip);
...
{
  double res = ((time * (double) 1e+9) / ((double)fRepeat * (double)fMaxPoints));
  return res;
}

double ShapeTester::MeasureTest (int (ShapeTester::*funcPtr)(int), const string &afMethod)
{
  Flush("Measuring performance of fMethod "+afMethod+"\n");
  ...
  ss << "Time per one fRepeat: " << realTime / fRepeat << "s\n";
  ss << "Nanoseconds per one fMethod call: " << NormalizeToNanoseconds(realTime) << "\n";
  FlushSS(ss);
...
int ShapeTester::CountDoubleDifferences(const vector<double> &differences, const vector<double> &values1, const vector<double> &values2)
{
  ...
  for (int i = 0; i < fMaxPoints; i++)
  {
    ...
    if (difference > std::abs (fDifferenceTolerance*value1))
    {
      if (++countOfDifferences <= 10) ss << "Different point found: index " << i <<
                                        "; point coordinates:" << PrintCoordinates(fPoints[i], ",") <<
                                        "; direction coordinates:" << PrintCoordinates(fDirections[i], ",") <<
...
int ShapeTester::CountDoubleDifferences(const vector<double> &differences)
{
  ...
  for (int i = 0; i < fMaxPoints; i++)
  {
    ...
    if (difference > fDifferenceTolerance) countOfDifferences++;
  }
  stringstream ss;
  ss << "Number of differences is " << countOfDifferences << "\n";
  FlushSS(ss);
...
int ShapeTester::SaveResultsToFile(const string &fMethod1)
{
  ...
  if (file.is_open())
  {
    ...
    for (int i = 0; i < fMaxPoints; i++)
    {
      ...
      file << PrintCoordinates(fPoints[i], spacer, prec) << spacer << PrintCoordinates(fDirections[i], spacer, prec) << spacer;
      if (saveVectors) file << PrintCoordinates(fResultVectorUSolids[i], spacer, prec) << "\n";
      else file <<  fResultDoubleUSolids[i] << "\n";
    }
    return 0;
  }
...
int ShapeTester::TestMethod(int (ShapeTester::*funcPtr)())
{
  ...
  if(fMethod != "XRayProfile"){
    ...
    std::cout << "%             ";
    std::cout << "surface=" << fMaxPointsSurface << ", inside=" << fMaxPointsInside << ", outside=" <<
      fMaxPointsOutside << "\n";
  }
  std::cout << "%     "<<std::endl;
...
int ShapeTester::Run(VUSolid *testVolume)
{
  ...
  if (fMethod == "all") errCode+= TestMethodAll();
  else if (funcPtr) errCode+= TestMethod(funcPtr);
  else std::cout<< "Method " << fMethod << " is not supported" << std::endl;
                               ...
                               std::string comment)//, std::ostream &fLogger )
{
  ...
  std::cout << "% " << comment;
  if (errors->sNUsed == 5) std::cout << " (any further such errors suppressed)";
