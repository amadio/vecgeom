...
TGeoNode *FindDaughter(TGeoVolume *vol, std::string &name, int &index)
{
  ...
  if (found) {
    for (int i=0; i<nd; ++i) {
      if (vol->GetNode(i) != found)
        vol->GetNode(i)->GetVolume()->SetVisibility(false);
    }
  }
  return found;
}
...
*/
  ...
  LogicalVolume *converted = (daughter) ? RootGeoManager::Instance().Convert(founddaughter)
                                        : RootGeoManager::Instance().Convert(foundvolume);
  ...
  if (!daughter) foundvolume->DrawOnly();
  else foundvolume->Draw();
