...
UTrap::UTrap(const std::string &pName, double pDz, double pTheta, double pPhi, double pDy1, double pDx1, double pDx2,
    ...
    : VUSolid(pName)
{
  if (pDz <= 0 || pDy1 <= 0 || pDx1 <= 0 || pDx2 <= 0 || pDy2 <= 0 || pDx3 <= 0 || pDx4 <= 0) {
    ...
    message << "Invalid length parameters for Solid: " << GetName() << std::endl
            << "				X - " << pDx1 << ", " << pDx2 << ", " << pDx3 << ", " << pDx4 << std::endl
            << "					Y - " << pDy1 << ", " << pDy2 << std::endl
            << "					Z - " << pDz;
    UUtils::Exception("UTrap::UTrap()", "GeomSolids0002", UFatalErrorInArguments, 1, message.str().c_str());
  }
...
UTrap::UTrap(const std::string &pName, const UVector3 pt[8]) : VUSolid(pName)
{
  SetPlanes(pt);
  fCubicVolume = 0.;
  fSurfaceArea = 0.;
...
UTrap::UTrap(const std::string &pName, double pZ, double pY, double pX, double pLTX) : VUSolid(pName)
{
  bool good;
  ...
  pt[0] = UVector3(-fDz * fTthetaCphi - fDy1 * fTalpha1 - fDx1, -fDz * fTthetaSphi - fDy1, -fDz);
  pt[1] = UVector3(-fDz * fTthetaCphi - fDy1 * fTalpha1 + fDx1, -fDz * fTthetaSphi - fDy1, -fDz);
  pt[2] = UVector3(-fDz * fTthetaCphi + fDy1 * fTalpha1 - fDx2, -fDz * fTthetaSphi + fDy1, -fDz);
  pt[3] = UVector3(-fDz * fTthetaCphi + fDy1 * fTalpha1 + fDx2, -fDz * fTthetaSphi + fDy1, -fDz);
  pt[4] = UVector3(+fDz * fTthetaCphi - fDy2 * fTalpha2 - fDx3, +fDz * fTthetaSphi - fDy2, +fDz);
  pt[5] = UVector3(+fDz * fTthetaCphi - fDy2 * fTalpha2 + fDx3, +fDz * fTthetaSphi - fDy2, +fDz);
  pt[6] = UVector3(+fDz * fTthetaCphi + fDy2 * fTalpha2 - fDx4, +fDz * fTthetaSphi + fDy2, +fDz);
  pt[7] = UVector3(+fDz * fTthetaCphi + fDy2 * fTalpha2 + fDx4, +fDz * fTthetaSphi + fDy2, +fDz);
  ...
  if (!good) {
    std::ostringstream message;
    message << "Face at ~+X not planar for Solid: " << GetName();
    UUtils::Exception("UTrap::UTrap()", "GeomSolids0002", UFatalError, 1, message.str().c_str());
  }
  fCubicVolume = 0.;
  fSurfaceArea = 0.;
}
...
UTrap::UTrap(const std::string &pName, double pDx1, double pDx2, double pDy1, double pDy2, double pDz) : VUSolid(pName)
{
  bool good;
...
UTrap::UTrap(const std::string &pName, double pDx, double pDy, double pDz, double pAlpha, double pTheta, double pPhi)
    : VUSolid(pName)
{
  bool good;
...
UTrap::UTrap(const std::string &pName)
    : VUSolid(pName), fDz(1.), fTthetaCphi(0.), fTthetaSphi(0.), fDy1(1.), fDx1(1.), fDx2(1.), fTalpha1(0.), fDy2(1.),
      fDx3(1.), fDx4(1.), fTalpha2(0.)
{
  MakePlanes();
  fCubicVolume = 0.;
  fSurfaceArea = 0.;
...
UTrap::UTrap(const UTrap &rhs)
    : VUSolid(rhs), fDz(rhs.fDz), fTthetaCphi(rhs.fTthetaCphi), fTthetaSphi(rhs.fTthetaSphi), fDy1(rhs.fDy1),
      fDx1(rhs.fDx1), fDx2(rhs.fDx2), fTalpha1(rhs.fTalpha1), fDy2(rhs.fDy2), fDx3(rhs.fDx3), fDx4(rhs.fDx4),
...
void UTrap::SetAllParameters(double pDz, double pTheta, double pPhi, double pDy1, double pDx1, double pDx2,
                             double pAlp1, double pDy2, double pDx3, double pDx4, double pAlp2)
{
  if (pDz <= 0 || pDy1 <= 0 || pDx1 <= 0 || pDx2 <= 0 || pDy2 <= 0 || pDx3 <= 0 || pDx4 <= 0) {
    std::ostringstream message;
    message << "Invalid Length Parameters for Solid: " << GetName() << std::endl
            ...
            << "					Y - " << pDy1 << ", " << pDy2 << std::endl
            << "					Z - " << pDz;
    UUtils::Exception("UTrap::SetAllParameters()", "GeomSolids0002", UFatalErrorInArguments, 1, message.str().c_str());
  }
  fCubicVolume = 0.;
  fSurfaceArea = 0.;
  fDz          = pDz;
...
void UTrap::SetPlanes(const UVector3 pt[8])
{
  ...
  if (!(pt[0].z() < 0 && pt[0].z() == pt[1].z() && pt[0].z() == pt[2].z() && pt[0].z() == pt[3].z() && pt[4].z() > 0 &&
        pt[4].z() == pt[5].z() && pt[4].z() == pt[6].z() && pt[4].z() == pt[7].z() &&
        std::fabs(pt[0].z() + pt[4].z()) < VUSolid::Tolerance() && pt[0].y() == pt[1].y() && pt[2].y() == pt[3].y() &&
        pt[4].y() == pt[5].y() && pt[6].y() == pt[7].y() &&
        std::fabs(pt[0].y() + pt[2].y() + pt[4].y() + pt[6].y()) < VUSolid::Tolerance() &&
        std::fabs(pt[0].x() + pt[1].x() + pt[4].x() + pt[5].x() + pt[2].x() + pt[3].x() + pt[6].x() + pt[7].x()) <
            VUSolid::Tolerance())) {
    std::ostringstream message;
  ...
  if (!good) {
    ...
    UUtils::Exception("UTrap::UTrap()", "GeomSolids0002", UFatalError, 1, "Face at ~-Y not planar.");
  }
...
bool UTrap::MakePlanes()
{
  ...
  if (!good) {
    std::ostringstream message;
    message << "Face at ~-Y not planar for Solid: " << GetName();
    UUtils::Exception("UTrap::MakePlanes()", "GeomSolids0002", UFatalError, 1, message.str().c_str());
  }
...
bool UTrap::MakePlane(const UVector3 &p1, const UVector3 &p2, const UVector3 &p3, const UVector3 &p4,
                      UTrapSidePlane &plane)
{
  ...
  if (std::fabs(Vcross.Dot(v14) / (Vcross.Mag() * v14.Mag())) > kCoplanar_Tolerance) {
    ...
    if (sd > 0) {
      ...
      std::ostringstream message;
      message << "Invalid parameters: norm.mod() <= 0, for Solid: " << GetName();
      UUtils::Exception("UTrap::MakePlanes()", "GeomSolids0002", UFatalError, 1, message.str().c_str());
    }
...
VUSolid::EnumInside UTrap::Inside(const UVector3 &p) const
{
  ...
  if (std::fabs(p.z()) <= fDz - VUSolid::Tolerance() * 0.5) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (Dist > VUSolid::Tolerance() * 0.5)
        return in = EnumInside::eOutside;
      else if (Dist > -VUSolid::Tolerance() * 0.5)
...
bool UTrap::Normal(const UVector3 &p, UVector3 &aNormal) const
{
  ...
  for (i = 0; i < 4; i++) {
    dist = std::fabs(fPlanes[i].a * p.x() + fPlanes[i].b * p.y() + fPlanes[i].c * p.z() + fPlanes[i].d);
    if (dist < safe) {
      safe = dist;
    }
  }
  ...
  distmy = std::fabs(fPlanes[0].a * p.x() + fPlanes[0].b * p.y() + fPlanes[0].c * p.z() + fPlanes[0].d);

  disty = std::fabs(fPlanes[1].a * p.x() + fPlanes[1].b * p.y() + fPlanes[1].c * p.z() + fPlanes[1].d);

  distmx = std::fabs(fPlanes[2].a * p.x() + fPlanes[2].b * p.y() + fPlanes[2].c * p.z() + fPlanes[2].d);

  distx = std::fabs(fPlanes[3].a * p.x() + fPlanes[3].b * p.y() + fPlanes[3].c * p.z() + fPlanes[3].d);
  ...
  if (noSurfaces == 0) {
#ifdef UDEBUG
    UUtils::Exception("UTrap::SurfaceNormal(p)", "GeomSolids1002", UWarning, 1, "Point p is not on surface !?");
#endif
...
double UTrap::DistanceToIn(const UVector3 &p, const UVector3 &v, double) const
{
...
double UTrap::DistanceToOut(const UVector3 &p, const UVector3 &v, UVector3 &aNormalVector, bool &aConvex, double) const
{
  ...
  default:
    ...
    UUtils::Exception("UTrap::DistanceToOut(p,v,..)", "GeomSolids1002", UWarning, 1, message.str().c_str());
    break;
  }
  return snxt;
}
...
double UTrap::SafetyFromInside(const UVector3 &p, bool /*precise*/) const
{
  ...
  if (Inside(p) == EnumInside::eOutside) {
    ...
    UUtils::Exception("UTrap::DistanceToOut(p)", "GeomSolids1002", UWarning, 1, "Point p is outside !?");
  }
#endif
...
std::ostream &UTrap::StreamInfo(std::ostream &os) const
{
  ...
  os << "-----------------------------------------------------------\n"
     ...
     << "				" << fPlanes[0].a << " X + " << fPlanes[0].b << " Y + " << fPlanes[0].c << " Z + " << fPlanes[0].d
     << " = 0\n"
     << "				" << fPlanes[1].a << " X + " << fPlanes[1].b << " Y + " << fPlanes[1].c << " Z + " << fPlanes[1].d
     << " = 0\n"
     << "				" << fPlanes[2].a << " X + " << fPlanes[2].b << " Y + " << fPlanes[2].c << " Z + " << fPlanes[2].d
     << " = 0\n"
     << "				" << fPlanes[3].a << " X + " << fPlanes[3].b << " Y + " << fPlanes[3].c << " Z + " << fPlanes[3].d
     << " = 0\n"
     << "-----------------------------------------------------------\n";
  os.precision(oldprc);
...
UVector3 UTrap::GetPointOnPlane(UVector3 p0, UVector3 p1, UVector3 p2, UVector3 p3, double &area) const
{
  ...
  Area = UVector3(w.y() * v.z() - w.z() * v.y(), w.z() * v.x() - w.x() * v.z(), w.x() * v.y() - w.y() * v.x());

  aOne = 0.5 * Area.Mag();

  Area = UVector3(t.y() * u.z() - t.z() * u.y(), t.z() * u.x() - t.x() * u.z(), t.x() * u.y() - t.y() * u.x());
...
void UTrap::Extent(UVector3 &aMin, UVector3 &aMax) const
{
  ...
  temp[0] = pt[0].x() + (pt[4].x() - pt[0].x()) * (aMin.z() - pt[0].z()) / (pt[4].z() - pt[0].z());
  temp[1] = pt[0].x() + (pt[4].x() - pt[0].x()) * (aMax.z() - pt[0].z()) / (pt[4].z() - pt[0].z());
  temp[2] = pt[2].x() + (pt[6].x() - pt[2].x()) * (aMin.z() - pt[2].z()) / (pt[6].z() - pt[2].z());
  temp[3] = pt[2].x() + (pt[6].x() - pt[2].x()) * (aMax.z() - pt[2].z()) / (pt[6].z() - pt[2].z());
  temp[4] = pt[3].x() + (pt[7].x() - pt[3].x()) * (aMin.z() - pt[3].z()) / (pt[7].z() - pt[3].z());
  temp[5] = pt[3].x() + (pt[7].x() - pt[3].x()) * (aMax.z() - pt[3].z()) / (pt[7].z() - pt[3].z());
  temp[6] = pt[1].x() + (pt[5].x() - pt[1].x()) * (aMin.z() - pt[1].z()) / (pt[5].z() - pt[1].z());
  temp[7] = pt[1].x() + (pt[5].x() - pt[1].x()) * (aMax.z() - pt[1].z()) / (pt[5].z() - pt[1].z());
  ...
  // Y axis
  temp[0] = pt[0].y() + (pt[4].y() - pt[0].y()) * (aMin.z() - pt[0].z()) / (pt[4].z() - pt[0].z());
  temp[1] = pt[0].y() + (pt[4].y() - pt[0].y()) * (aMax.z() - pt[0].z()) / (pt[4].z() - pt[0].z());
  temp[2] = pt[2].y() + (pt[6].y() - pt[2].y()) * (aMin.z() - pt[2].z()) / (pt[6].z() - pt[2].z());
  temp[3] = pt[2].y() + (pt[6].y() - pt[2].y()) * (aMax.z() - pt[2].z()) / (pt[6].z() - pt[2].z());
