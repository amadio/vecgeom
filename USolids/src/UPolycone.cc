...
UPolycone::UPolycone(const std::string &name, double phiStart, double phiTotal, int numZPlanes, const double zPlane[],
                     const double rInner[],
                     const double rOuter[])
...
UPolycone::UPolycone(const std::string &name, double phiStart, double phiTotal, int numRZ, const double r[],
                     const double z[])
    : VUSolid(name)
{
  ...
  if (!convertible) {
    ...
    UUtils::Exception("UPolycone::UPolycone()", "GeomSolids0002", UFatalError, 1, message.str().c_str());
  } else {
    std::cout << "INFO: Converting polycone " << GetName() << std::endl
              << "to optimized polycone with (Rmin,Rmaz,Z) parameters !" << std::endl;
    double *Z, *R1, *R2;
...
//
void UPolycone::Init(double phiStart, double phiTotal, int numZPlanes, const double zPlane[], const double rInner[],
                     const double rOuter[])
{
  ...
  for (int j = 1; j < numZPlanes; j++) {
    ...
    if (rInner[j] > rOuter[j]) {
      std::ostringstream message;
      message << "Cannot create Polycone with rInner > rOuter for the same Z" << std::endl
              << "        rInner > rOuter for the same Z !" << std::endl
              << "        rMin[" << j << "] = " << rInner[j] << " -- rMax[" << j << "] = " << rOuter[j];
      UUtils::Exception("UPolycone::UPolycone()", "GeomSolids0002", UFatalErrorInArguments, 1, message.str().c_str());
    }
  }
  //
  ...
  for (i = 0; i < numZPlanes; i++) {
    if ((i < numZPlanes - 1) && (zPlane[i] == zPlane[i + 1])) {
      if ((rInner[i] > rOuter[i + 1]) || (rInner[i + 1] > rOuter[i])) {
        ...
        std::ostringstream message;
        message << "Cannot create a Polycone with no contiguous segments." << std::endl
                << "Segments are not contiguous !" << std::endl
                << " rMin[" << i << "] = " << rInner[i] << " -- rMax[" << i + 1 << "] = " << rOuter[i + 1] << std::endl
    ...
    if (i > 0) {
      if (((z > prevZ) && (dirZ > 0)) || ((z < prevZ) && (dirZ < 0))) {
        if (dirZ * (z - prevZ) < 0) {
          std::ostringstream message;
          message << "Cannot create a Polycone with different Z directions.Use GenericPolycone." << std::endl
                  << "ZPlane is changing direction  !" << std::endl
                  << "  zPlane[0] = " << zPlane[0] << " -- zPlane[1] = " << zPlane[1] << std::endl
                  << "  zPlane[" << i - 1 << "] = " << zPlane[i - 1] << " -- rPlane[" << i << "] = " << zPlane[i];
          UUtils::Exception("UPolycone::UPolycone()", "GeomSolids0002", UFatalErrorInArguments, 1,
                            message.str().c_str());
        }
        VUSolid *solid;
  ...
  if (rz->Amin() < 0.0) {
    ...
    UUtils::Exception("UPolycone::Init()", "GeomSolids0002", UFatalErrorInArguments, 1, message.str().c_str());
  }
...
VUSolid::EnumInside UPolycone::InsideSection(int index, const UVector3 &p) const
{
  ...
  if (r2 < 1e-10) {
    ...
    if ((!phiIsOpen) && (rMinMinus <= 0)) {
      if (pos != EnumInside::eSurface) {
        pos = EnumInside::eInside;
      }
    }
    if (rMinMinus <= 0) {
...
VUSolid::EnumInside UPolycone::Inside(const UVector3 &p) const
{
  ...
  return (nextPos == EnumInside::eSurface || pos == EnumInside::eSurface) ? EnumInside::eSurface : EnumInside::eOutside;
}

double UPolycone::DistanceToIn(const UVector3 &p, const UVector3 &v, double) const
{
  double shift = fZs[0] + fBox.GetZHalfLength();
  UVector3 pb(p.x(), p.y(), p.z() - shift);
  ...
  do {
    ...
    if (distance < UUtils::kInfinity || !increment) break;
    index += increment;
    pb.z() += section.shift;
...
double UPolycone::DistanceToOut(const UVector3 &p, const UVector3 &v, UVector3 &n, bool &convex, double) const
{
  UVector3 pn(p);
  ...
  do {
    ...
    if (std::fabs(distance) < 0.5 * fgTolerance) // Section Surface case
    {
      ...
      if ((index > 0) && (index < fMaxSection)) {
        ...
        if ((index == fMaxSection) && (increment < 0)) {
          index1 += increment;
        }
      }
  ...
  if (convex) // Check final convexity for dz
  {
    ...
    if ((index < fMaxSection) && (index > 0)) {
      ...
      if (dz1 < halfTolerance) {
        convex = false;
      }
      if (dz2 < halfTolerance) {
        convex = false;
      }
    } else {
      if (index >= fMaxSection) {
        if (std::fabs(pn.z() - fZs[fMaxSection]) < halfTolerance) {
          convex = false;
        }
      } else {
        if (index <= 0) {
          if (std::fabs(pn.z() - fZs[1]) < halfTolerance) {
            convex = false;
          }
        }
      }
...
double UPolycone::SafetyFromInside(const UVector3 &p, bool) const
{
  ...
  for (int i = index + 1; i <= fMaxSection; ++i) {
    ...
    if (dz1 < dz2) {
      safeR = std::sqrt(safeR * safeR + dz1 * dz1);
    } else {
      safeR = std::sqrt(safeR * safeR + dz1 * dz1);
    }
    if (safeR < dz1) {
...
{
  if (!aAccurate) return enclosingCylinder->SafetyFromOutside(p);
...
double UPolycone::SurfaceArea()
{
  if (fSurfaceArea != 0) {
    ...
    areas.push_back(UUtils::kPi *
                    (UUtils::sqr(fOriginalParameters->Rmax[0]) - UUtils::sqr(fOriginalParameters->Rmin[0])));

    for (i = 0; i < numPlanes - 1; i++) {
      Area = (fOriginalParameters->Rmin[i] + fOriginalParameters->Rmin[i + 1]) *
             std::sqrt(UUtils::sqr(fOriginalParameters->Rmin[i] - fOriginalParameters->Rmin[i + 1]) +
                       UUtils::sqr(fOriginalParameters->fZValues[i + 1] - fOriginalParameters->fZValues[i]));

      Area += (fOriginalParameters->Rmax[i] + fOriginalParameters->Rmax[i + 1]) *
              std::sqrt(UUtils::sqr(fOriginalParameters->Rmax[i] - fOriginalParameters->Rmax[i + 1]) +
                        UUtils::sqr(fOriginalParameters->fZValues[i + 1] - fOriginalParameters->fZValues[i]));

      Area *= 0.5 * (endPhi - startPhi);

      if (fOriginalParameters->fOpeningAngle < 2 * UUtils::kPi) {
        Area += std::fabs(fOriginalParameters->fZValues[i + 1] - fOriginalParameters->fZValues[i]) *
                (fOriginalParameters->Rmax[i] + fOriginalParameters->Rmax[i + 1] - fOriginalParameters->Rmin[i] -
...
//
UVector3 UPolycone::GetPointOnCone(double fRmin1, double fRmax1, double fRmin2, double fRmax2, double zOne, double zTwo,
                                   double &totArea) const
{
  ...
  if ((chose >= 0) && (chose < Aone)) {
    if (fRmax1 != fRmax2) {
      ...
      point = UVector3(rone * cosu * (qone - zRand), rone * sinu * (qone - zRand), zRand);
    } else {
      point = UVector3(fRmax1 * cosu, fRmax1 * sinu, UUtils::Random(-1. * afDz, afDz));
    }
  } else if (chose >= Aone && chose < Aone + Atwo) {
    if (fRmin1 != fRmin2) {
      zRand = UUtils::Random(-1. * afDz, afDz);
      point = UVector3(rtwo * cosu * (qtwo - zRand), rtwo * sinu * (qtwo - zRand), zRand);

    } else {
      point = UVector3(fRmin1 * cosu, fRmin1 * sinu, UUtils::Random(-1. * afDz, afDz));
    }
    ...
    point  = UVector3(rRand1 * std::cos(startPhi), rRand1 * std::sin(startPhi), zRand);
  } else {
    zRand  = UUtils::Random(-1. * afDz, afDz);
    rmin   = fRmin2 - ((zRand - fDz) / (2. * fDz)) * (fRmin1 - fRmin2);
    rmax   = fRmax2 - ((zRand - fDz) / (2. * fDz)) * (fRmax1 - fRmax2);
    rRand1 = std::sqrt(UUtils::Random() * (UUtils::sqr(rmax) - UUtils::sqr(rmin)) + UUtils::sqr(rmin));
    point  = UVector3(rRand1 * std::cos(endPhi), rRand1 * std::sin(endPhi), zRand);
  }
...
//
UVector3 UPolycone::GetPointOnTubs(double fRMin, double fRMax, double zOne, double zTwo, double &totArea) const
{
  double xRand, yRand, zRand, phi, cosphi, sinphi, chose, aOne, aTwo, aFou, rRand, fDz, fSPhi, fDPhi;
  ...
  if (startPhi == 0 && endPhi == 2 * UUtils::kPi) aFou = 0;
...
// Auxiliary method for GetPoint On Surface
//
UVector3 UPolycone::GetPointOnRing(double fRMin1, double fRMax1, double fRMin2, double fRMax2, double zOne) const
{
...
// Auxiliary method for Get Point On Surface
//
UVector3 UPolycone::GetPointOnCut(double fRMin1, double fRMax1, double fRMin2, double fRMax2, double zOne, double zTwo,
                                  double &totArea) const
{
  if (zOne == zTwo) {
...
UVector3 UPolycone::GetPointOnSurface() const
{
  ...
  rRand = fOriginalParameters->Rmin[0] +
          ((fOriginalParameters->Rmax[0] - fOriginalParameters->Rmin[0]) * std::sqrt(UUtils::Random()));
  ...
  if ((chose >= 0.) && (chose < areas[0])) {
    return UVector3(rRand * cosphi, rRand * sinphi, fOriginalParameters->fZValues[0]);
  }
  ...
  for (i = 0; i < numPlanes - 1; i++) {
    ...
    if (chose >= Achose1 && chose < Achose2) {
      return GetPointOnCut(fOriginalParameters->Rmin[i], fOriginalParameters->Rmax[i], fOriginalParameters->Rmin[i + 1],
                           fOriginalParameters->Rmax[i + 1], fOriginalParameters->fZValues[i],
  ...
  return UVector3(rRand * cosphi, rRand * sinphi, fOriginalParameters->fZValues[numPlanes - 1]);
}
...
bool UPolycone::SetOriginalParameters(UReduciblePolygon *rz)
{
    ...
    if (Zright > Zleft) {
      ...
      if (std::fabs(difZl) < frTolerance) {
        if (std::fabs(difZr) < frTolerance) {
          ...
          Rmin.push_back(corners[inextl].r);
          Rmax.push_back(corners[icurr].r +
                         (Zleft - corners[icurr].z) / difZr * (corners[inextr].r - corners[icurr].r));
        }
    ...
    } else // Zright<Zleft
    {
      ...
      if (std::fabs(difZr) < frTolerance) {
        if (std::fabs(difZl) < frTolerance) {
          ...
          Rmin.push_back(corners[icurl].r +
                         (Zright - corners[icurl].z) / difZl * (corners[inextl].r - corners[icurl].r));
          Rmax.push_back(corners[inextr].r);
        }
        icurr++;
      } // plate
  ...
  } else // Set parameters(r,z) with Rmin==0 as convention
  {
    ...
    UUtils::Exception("UPolycone::SetOriginalParameters()", "GeomSolids0002", UWarning, 1, "can not convert");
