...
using vecgeom::cxx::PolyconeHistorical;
...
public:
  ...
  UPolyconeHistorical *GetOriginalParameters() const
  {
    ...
    OriginalParameters->fNumZPlanes                  = vgPolyconeOriginalParameters->fHNum_z_planes;
    OriginalParameters->fZValues.resize(OriginalParameters->fNumZPlanes);
    OriginalParameters->Rmin.resize(OriginalParameters->fNumZPlanes);
    OriginalParameters->Rmax.resize(OriginalParameters->fNumZPlanes);
    for (int i = 0; i < OriginalParameters->fNumZPlanes; i++) {
      OriginalParameters->fZValues[i] = vgPolyconeOriginalParameters->fHZ_values[i];
      OriginalParameters->Rmin[i]     = vgPolyconeOriginalParameters->fHRmin[i];
      OriginalParameters->Rmax[i]     = vgPolyconeOriginalParameters->fHRmax[i];
    }
...
public:                            // with description
  ...
  UVector3 GetPointOnCone(double fRmin1, double fRmax1, double fRmin2, double fRmax2, double zOne, double zTwo,
                          double &totArea) const;

  UVector3 GetPointOnTubs(double fRMin, double fRMax, double zOne, double zTwo, double &totArea) const;

  UVector3 GetPointOnCut(double fRMin1, double fRMax1, double fRMin2, double fRMax2, double zOne, double zTwo,
                         double &totArea) const;

  UVector3 GetPointOnRing(double fRMin, double fRMax, double fRMin2, double fRMax2, double zOne) const;

  inline double GetStartPhi() const { return startPhi; }

  inline double GetEndPhi() const { return endPhi; }

  inline bool IsOpen() const { return phiIsOpen; }

  inline bool IsGeneric() const { return false; }

  inline int GetNumRZCorner() const { return numCorner; }

  inline UPolyconeSideRZ GetCorner(int index) const { return corners[index]; }

  inline UPolyconeHistorical *GetOriginalParameters() const { return fOriginalParameters; }
...
protected: // without description
  ...
  inline double SafetyFromInsideSection(int index, const double rho, const UVector3 &p) const
  {
  ...
  //
  inline double SafetyFromOutsideSection(int index, const double rho, const UVector3 &p) const
  {
    const UPolyconeSection &section = fSections[index];
