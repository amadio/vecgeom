...
using SpecializedPolycone = SpecializedVolImplHelper<PolyconeImplementation<polyconeTypeT>>;

using SimplePolycone = SpecializedPolycone<ConeTypes::UniversalCone>;

template <typename Type>
VECCORE_ATT_DEVICE VPlacedVolume *SUnplacedPolycone<Type>::Create(LogicalVolume const *const logical_volume,
...
{
  (void)placement;
  return new SpecializedPolycone<Type>(logical_volume, transformation
#ifdef VECCORE_CUDA
                                       ,
                                       id, copy_no, child_id
#endif
